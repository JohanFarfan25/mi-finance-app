...
export class SettingsPage implements OnInit {
  ...
  private async handleCurrentPinVerification(
    currentPin: string,
  ): Promise<void> {
    if (!currentPin) {
  ...
  private async handleNewPinSave(
    newPin: string,
    confirmPin: string,
  ): Promise<void> {
    if (!newPin) {
  ...
  async exportDataToPdf() {
    ...
    try {
      ...
      const budgets = await this.budgetService.getBudgetsByUser(
        this.currentUser.id,
      );
      ...
      const formatMoney = (value: number) =>
        value.toLocaleString() + ' ' + this.currency;
  ...
  async onSettingTap(item: any) {
    ...
    if (item.label === 'Limpiar Datos') {
      const alert = await this.alertCtrl.create({
        ...
        message:
          'Esto eliminará todas tus transacciones, categorías y presupuestos. ¿Continuar?',
        buttons: [
          { text: 'Cancelar', role: 'cancel' },
          {
            text: 'Eliminar',
  ...
  async onCurrencyChange(event: any) {
    ...
    if (this.currentUser && this.currentUser.currency !== newCurrency) {
      this.currentUser.currency = newCurrency;
      ...
      const alert = await this.alertCtrl.create({
        header: 'Moneda actualizada',
        message: `La moneda se ha cambiado a ${newCurrency}`,
        buttons: ['OK'],
      });
      await alert.present();
    }
